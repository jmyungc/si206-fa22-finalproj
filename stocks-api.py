...
def create_first_stock_table(cur, conn, stock_tup_list):
    ...
    if (max_date_id[0] == 104 and stock_index <= 105):
        while count < 25 and stock_index < len(stock_tup_list):
            ...
            count += 1
            stock_index += 1
    elif (stock_count == 105):
        print(stock_count)
        print("First stock complete")
    else:
        return
    return
...
def add_second_stock(cur, conn, stock_tup_list):
    ...
    if (stocks_in_table[0] >= 105 and stocks_in_table[0] < 210):
        while count < 25 and start_index < len(stock_tup_list):
            ...
            start_index += 1
            cur.execute('SELECT COUNT(*) FROM Stocks')
    elif (stocks_in_table[0] >= 210):
        print(stocks_in_table)
        print("Second stock complete")
    else:
        return
    return
...
def add_third_stock(cur, conn, stock_tup_list):
    ...
    if (stocks_in_table[0] >= 210 and stocks_in_table[0] < 315):
        while count < 25 and start_index < len(stock_tup_list):
            ...
            count += 1
            start_index += 1
    elif (stocks_in_table[0]==315):
        print(stocks_in_table)
        print("Third stock complete")
    else:
        return
    return

def main():
    cur, conn = create_database('stock_lows_highs.db')
