...
def create_week_id_table(cur, conn, week_list):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS Stocks (stock TEXT, low INTEGER, high INTEGER, week_id INTEGER, difference INTEGER, FOREIGN KEY (week_id) REFERENCES Dates(id))')
...
def create_first_stock_table(cur, conn, stock_tup_list):
    ...
    start_index = stock_count[0]
    count = 0
    
    if (max_date_id[0] == 104 and start_index <= 105):
        while count < 25 and start_index < len(stock_tup_list):
            diff = round((float(stock_tup_list[start_index][3]) - float(stock_tup_list[start_index][2])), 2)

            cur.execute('INSERT OR IGNORE INTO Stocks (stock, low, high, difference, week_id) VALUES (?,?,?,?,?)',(stock_tup_list[0][1], stock_tup_list[start_index][2], stock_tup_list[start_index][3], diff, stock_tup_list[start_index][0]))
